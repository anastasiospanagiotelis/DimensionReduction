...
Carry out principal components on the World Bank data (`WorldBankClean.csv`) and answer the following questions. To answer the questions run the command `?prcomp` to see the help documentation of the function used to create principal components.
...
`r if(sols) '*The data are all measured in different units, for example FX.OWN.TOTL.ZS (Account ownership at a financial institution or with a mobile-money-service provider) is measured in percentage of population aged above 15 years, while FB.ATM.TOTL.P5 (Automated teller machines (ATMs)) is measured in per 100,000 adults while TX.VAL.TECH.CD (High-technology exports) is measured in $US. The variance of each variable will be sensitive to the units of measurement, for example measuring FX.OWN.TOTL.ZS in per 100,000 adults and TX.VAL.TECH.CD in millions of $US will change the relative variance.*'`
...
`r if(sols) '*The output can be seen from the output of the summary function. A piece of code that gives the anser is*'`
...
4. Plot the first two principal components of the data as a scatterplot.  Use the country abbreviation rather than a points
...
`r if(sols) '*Note that the argument coord_equal is used for the plot. This ensures that 1 unit on the x axis is the sample length as 1 unit on the y axis. Most software that creates a scatterplot will not have this property by default.*'`
...
1. Using the household with ID=4669 for th Irish smart meter data, find the location and scale parameter from fitting electricity demand for each time of week with a lognormal distribution 
...
sm%>%filter(id==4669)%>%
  ...
  group_by(tow)%>%
  summarise(mu=mean(logd,na.rm=TRUE),sigma2=var(logd,na.rm=TRUE))->pars
```
...
$$\log\sigma^2_Q-\log(\sigma^2_P)+\frac{\sigma^2_P+(\mu_P-\mu_Q)^2}{2\sigma^2_Q}-0.5$$
...
3. Construct similar to the one on [slide 20 of Lecture 3](https://anastasiospanagiotelis.netlify.app/teaching/amsi2021/03mds/03mds#52). Use the 
...
4. There are six points with a very high value on the first coordinate and a low value on the second coordinate. What do he points in this underlying group have in common. HINT: To investigate use the `arrange` function

`r if(sols) '*Using the arrange function we can see which values have the highest value of Component 1.*'`
...
`r if(sols) '*From this investigation we can conclude that this particular houshold has higher than average usage at 7:30 PM.*'`