...
$$\mathbf{S}=\sum_{j=1}^p \lambda_j\mathbf{w}_j\mathbf{w}_j'$$
- This can be approximated by

$$\mathbf{S}\approx\sum_{j=1}^{\color{blue}{k}} \lambda_j\mathbf{w}_j\mathbf{w}_j'$$
---
...
$$\mathbf{x}_i=\mathbf{W}\mathbf{c}_i+\boldsymbol{\xi}_i$$
--

- where
  + $\mathbf{x}_i$ is a $p\times 1$ data vector, 
  + $\mathbf{c}_i$ is a $k\times 1$ latent factor,
  + $\mathbf{W}$ are factor loadings,
