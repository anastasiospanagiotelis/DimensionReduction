...
$$d_{i,j}=\sqrt{(\bx_i-\bx_j)'(\bx_i-\bx_j)}=||\bx_i-\bx_j||_2$$
--
...
- The nearest neighbour (NN) of $\bx_i$ is $\bx_j$ such that $d_{i,j}\leq d_{i,k}$ for all $k\neq j$
--
...
- Alternative definition of neighbours of $\bx_i$ is all $\bx_j$ such that $d_{i,j}\leq \epsilon$
...
- Generally, work with orthonormal $\bv$, i.e. $\bv_i'\bv_i=1$ and $\bv_i'\bv_j=0$ for $i\neq j$
